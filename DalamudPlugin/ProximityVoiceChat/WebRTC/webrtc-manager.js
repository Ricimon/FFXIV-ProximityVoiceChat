...
class WebrtcManager {
     ...
     * @param {Function} signalingChannelSendTo - A delegate for sending a message through a signaling channel
     ...
     */
    constructor(ourPeerId, ourPeerType, signalingChannelSendTo, options, verbose = false) {
        this.ourPeerId = ourPeerId;
        this.ourPeerType = ourPeerType;
        this.signalingChannelSendTo = signalingChannelSendTo;
    ...
    updateNegotiationLogic(peer) {
        ...
        peerConnection.onicecandidate = ({ candidate }) => this.signalingChannelSendTo(peer.peerId, { action: "ice", ice: candidate });
        ...
        peerConnection.onnegotiationneeded = async () => {
            try {
                ...
                if (!peer.polite) {
                    ...
                    this.signalingChannelSendTo(peer.peerId, { action: "sdp", sdp: peerConnection.localDescription });
                }
            } catch (err) {
                console.error(err);
            } finally {
...
module.exports = function (options, callback) {
    var { ourPeerId, ourPeerType, signalingChannelSendTo, options, verbose } = options;
    var manager = new WebrtcManager(ourPeerId, ourPeerType, signalingChannelSendTo, options, verbose);
    callback(null, {
        processSignalingMessage: function (message, cb) {
            manager.signalingMessageHandler(message);
            cb();
        },
        processSignalingDisconnect: function (_, cb) {
            manager.signalingDisconnectHandler();
            cb();
        },
        getPeers: function (_, cb) {
            cb(manager.peers);
        }
    });
};
